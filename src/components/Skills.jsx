    ...
    <section id="skills" className="section-container scroll-mt-24 space-y-10">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {skillCategories.map((group) => (
          <div
            key={group.title}
            className="card-surface flex h-full flex-col p-5 sm:p-6 transition duration-300 hover:-translate-y-1 hover:border-cyan-400/25 hover:shadow-glow"
          >
            <div className="flex items-start justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-50">{group.title}</h3>
              <span className="text-xs uppercase tracking-[0.2em] text-slate-500">Core</span>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="card-surface flex h-full flex-col p-5 sm:p-6 transition duration-300 hover:-translate-y-1 hover:border-cyan-400/25 hover:shadow-glow">
          <div className="flex items-start justify-between gap-3">
            <h3 className="text-lg font-semibold text-slate-50">{cpHighlights.title}</h3>
            <span className="text-xs uppercase tracking-[0.2em] text-slate-500">CP</span>
          </div>
      ...
      <div className="card-surface p-5 sm:p-6 transition duration-300 hover:-translate-y-1 hover:border-cyan-400/25 hover:shadow-glow">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {skillLevels.map((skill) => (
            <div key={skill.name} className="rounded-xl border border-slate-800 bg-slate-900/60 px-4 py-3 shadow-sm shadow-cyan-500/5">
              <div className="text-sm font-semibold text-slate-50">{skill.name}</div>
              <div className="text-xs uppercase tracking-[0.2em] text-cyan-200">{skill.level}</div>
            </div>
          ))}
        </div>
      </div>
